...
class AdamModelTrainer():
    ...
    def load_data(self, set_standardize=False, test_size=0.2, val_size=0.2, subset_size=500):
...
if __name__ == "__main__":
    model = DnCNN()
    trainer = AdamModelTrainer(model=model, lr=1e-3)
    trainer.load_data()
    trainer.train(is_plot_graph=1)
    trainer.evaluate()
    trainer.visualize_denoising(num_images=10)
    #trainer.save_model('autoencoder_adam.pth')
